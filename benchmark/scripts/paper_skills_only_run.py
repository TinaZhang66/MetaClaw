#!/usr/bin/env python3
"""
paper_skills_only_run.py — paper-aligned skills_only benchmark runner on the full 30-day dataset.

Compared with skills_only_run.py:
  1. Uses paper-skills-only.yaml
...
class cfg:
    LOG_FILE = str(BENCH_ROOT / "logs" / "paper_skills_only_full_run" / "bench_run.log")
    BENCH_BIN = shutil.which("metaclaw-bench")
    BENCH_INPUT = str(BENCH_ROOT / "data" / "metaclaw-bench" / "all_tests_metaclaw.json")
    BENCH_OUTPUT = str(BENCH_ROOT / "results" / "paper_skills_only_full")
    BENCH_COUNT = 3
    API_KEY_SCRIPT = None
    PROXY_SCRIPT = str(SCRIPT_DIR / "proxy_run.py")
    PROXY_CONFIG = str(SCRIPT_DIR / "config" / "paper-skills-only.yaml")
...
def start_proxy(env: dict, config_path: str, port: int) -> subprocess.Popen:
    print(f"[proxy] 正在启动 proxy (paper skills_only, port={port})...")
...
def main():
    ...
    if not base_env.get("BENCHMARK_MODEL"):
        base_env["BENCHMARK_MODEL"] = (
            base_env.get("SKILLS_ONLY_MODEL_ID") or "Qwen3-8B"
        )
    ...
    temp_skill_dir = str(Path(tempfile.gettempdir()) / f"metaclaw_paper_skills_only_full_{ts}")
