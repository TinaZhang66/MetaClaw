#!/usr/bin/env python3
"""
static_skills_run.py — baseline + static skills benchmark runner.

Same as skills_only_run.py, except skill auto-evolution is disabled so the
benchmark measures pure skill injection without online skill rewriting.
"""
...
class cfg:
    LOG_FILE = str(BENCH_ROOT / "logs" / "static_skills_run" / "bench_run.log")
    BENCH_BIN = shutil.which("metaclaw-bench")
    BENCH_INPUT = str(BENCH_ROOT / "data" / "metaclaw-bench-small" / "all_tests_metaclaw.json")
    BENCH_OUTPUT = str(BENCH_ROOT / "results" / "static_skills")
    BENCH_COUNT = 3
    API_KEY_SCRIPT = None
    PROXY_SCRIPT = str(SCRIPT_DIR / "proxy_run.py")
    PROXY_CONFIG = str(SCRIPT_DIR / "config" / "static-skills.yaml")
...
def start_proxy(env: dict, config_path: str, port: int) -> subprocess.Popen:
    print(f"[proxy] 正在启动 proxy (static skills, port={port})...")
...
def main():
    ...
    temp_skill_dir = str(Path(tempfile.gettempdir()) / f"metaclaw_static_skills_{ts}")
